...
if number_of_files == 0:
    print(
        f"{bcolors.FAIL}No pdfs available to relabel.\nStopping execution...{bcolors.ENDC}"
    )
    sys.exit(0)
...
for pdf in PDFs:
    ...
    SHOULD_ADD_SIGNED_TAG = isDocuSigned(text)
    print(pdf)
...
print(
    f"{bcolors.OKGREEN}--- {number_of_files} files relabeled in %s seconds ---{bcolors.ENDC}"
